    ...
    <div style={{ fontFamily: FONT, background: C.bg, color: C.text, minHeight: "100vh" }}>
      ...
      <header style={{ padding: "20px 28px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: C.muted, fontFamily: MONO, marginTop: 2 }}>Serie INDEC ene 2017 – mar 2026 + Proxy R&V en vivo</div>
        </div>
      ...
      <div style={{ padding: "20px 28px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 12 }}>
          <MetricBox label="Nivel general INDEC" value={fmtIdx(RAW.nivelGeneral.indices[RAW.nivelGeneral.indices.length-1])} sub="Mar 2026" accent />
          <MetricBox label="Var. mensual INDEC" value={`+${fmt(latestVarVal)}%`} sub="Mar 2026 vs feb 2026" />
          ...
          <MetricBox label="Var. interanual" value={`+${fmt(latestYoYVal)}%`} sub="Mar 2026 vs mar 2025" />
          <MetricBox
            label="Inflación núcleo"
            value={`+${fmt(nucleoDisplay)}%`}
            sub={nucleoEsRV ? `${mesActualLabel} R&V (excl. reg.)` : "Mar 2026 INDEC (excl. estac. y reg.)"}
            nucleoStyle
          />
        </div>
          ...
          {proxyStatus==="ok" && proxy?.index ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10 }}>
              ...
              <ProxyCard
                label="Inflación núcleo"
                value={`+${fmt(nucleoDisplay)}%`}
                sub={nucleoEsRV ? `${mesActualLabel} R&V` : "Mar 2026 INDEC"}
              />
            </div>
          ) : proxyStatus==="loading" ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 16 }}>
                  ...
                  <div style={{ background: "rgba(167,139,250,0.08)", border: `1px solid rgba(167,139,250,0.25)`, borderRadius: 10, padding: "16px 18px" }}>
                    ...
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 4, fontFamily: MONO }}>
                      {nucleoEsRV ? `${mesActualLabel} R&V · excl. reg.` : "Mar 2026 INDEC · excl. estac. y reg."}
                    </div>
                  </div>
                </div>
          ...
          <>
                ...
                return (
                  <button key={i} onClick={() => setSelDiv(selDiv===i ? null : i)} style={{
                    ...
                    <div style={{ fontSize: 9, fontFamily: MONO, color: C.dim, marginTop: 1 }}>
                      {hasIntrames ? `Acum. ${mesActualLabel}` : "Mar 2026 INDEC"}
                    </div>
                  </button>
                );
              })}
            ...
            <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
              ...
              <div style={{ padding: "8px 12px", fontSize: 9, fontFamily: MONO, color: C.dim, borderTop: `1px solid ${C.border}` }}>
                Var. mensual: estimación R&V acumulada al {mesActualLabel} donde disponible (marcado R&V), resto mar 2026 INDEC · Mes anterior: pendiente datos INDEC abril 2026
              </div>
            </div>
          </>
        )}
        ...
        {tab === "categorias" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 16 }}>
              {[["Núcleo", C.teal, nucleoVal], ["Estacional", C.amber, 1.0], ["Regulados", C.purple, 5.4]].map(([name, color, last]) => (
                <div key={name} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}>
                  ...
                  <div style={{ fontSize: 10, color: C.muted, fontFamily: MONO }}>mensual mar 2026</div>
                </div>
              ))}
            </div>
